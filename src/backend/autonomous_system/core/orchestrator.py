    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.db_manager, "health_check"):
                db_details = await self.db_manager.health_check()
                if isinstance(db_details, dict):
                    db_healthy = db_details.get("status") == "healthy"
                else:
                    db_healthy = bool(db_details)
            else:
                # Avoid reconnecting if already connected
                if getattr(self.db_manager, "connection", None) is not None:
                    db_healthy = True
                else:
                    db_healthy = bool(await self.db_manager.connect())
            ...
            return {
                ...
                'checks': {
                    'database': {'healthy': db_healthy, 'details': db_details},
